"""
=============================================================================
Project Name        : PythoImport Version 7.4
Created by          : Hedi FEZAI
Date Creation       : 2022-11-10
Date Modification   : 2025-12-02
-----------------------------------------------------------------------------
Changelog :
...
v6.4 (2025-04-15)
    v Add support for FTP Over TLS (Port 21)
v6.5 (2025-05-07)
...
v6.9 (2025-11-07)
    v Added post transfer function in settings that runs on file rawlines after transfer and before import
    v Added support for insecure ftp (Port 21)
    v Added download from sftp by chunks to avoid connection drops by remote server when downloading big files
v7.0 (2025-11-11)
    v Fixed bug when one file corresponds to multiple masks
v7.1 (2025-11-12)
    v Better error handling when importing totally empty files (no columns to parse)
v7.2 (2025-11-12)
    v cleanCSV is now optional and by default disabled
v7.3 (2025-11-13)
    v improvements on dtypes function and float vs int values
v7.4 (2025-12-02)
    v Added SQL server authentication
WishList
	ToDo :
...
def cleanCsv(pathCsv, encoding='utf-8', delimiter=';', quotechar='"'):
    ...
    if rawLines:
        ...
        for line in rawLines:
            lineBuffer += line
            try:
                parsed = next(csv.reader([lineBuffer], delimiter=delimiter, quotechar=quotechar))
...
class DatabaseEngine:
    ...
    def create_connection(self):
        ...
        if SqlItems.get('authentication') == 'sql' :
            login = SqlItems.get('login')
            try:
                password = Fernet(getenv('FERNET_KEY')).decrypt(SqlItems.get('password')).decode()
            except:
                try:
                    password = base64.b64decode(SqlItems.get('password').encode("ascii")).decode("ascii").replace('\n', '')
                except Exception as pwd:
                    logToFile (logfile, 2, True, 'Error decrypting SQL password : ' + str(pwd))
            engine = sa.create_engine(f"mssql+pyodbc://{login}:{password}@{self.server},{SqlItems.get('sqlPort')}/{self.database}?trusted_connection=yes&driver={driverSA}&encrypt=no", echo = False)
            logToFile (logfile, 2, False, f'SQL Mode using {login}')
        else:
            ...
            logToFile (logfile, 2, False, 'Windows Mode')
        ...
        if SqlItems.get('authentication') == 'sql' :
            ...
            try:
                ...
                try:
                    ...
                    logToFile (logfile, 2, True, 'Error decrypting SQL password : ' + str(pwd))
            connection = pyodbc.connect(f"DRIVER={driverPO};SERVER={self.server},{SqlItems.get('sqlPort')};initial_catalog={self.database};UID={login};PWD={password};Encrypt=no;MARS_Connection=Yes;")
        else:
...
if __name__ == '__main__':
    ...
    def sqlcol(dfparam):
        ...
        for i, j in zip(dfparam.columns, dfparam.dtypes):
            # Si on ne force pas tout en NVARCHAR
            if not TsfItems['forceAlltoNVARCHAR']:
                # Si la colonne est définie dans dtypes
                if i in dtypes:
                    dtype_str = dtypes[i]
                    if dtype_str == 'float':
                        dtypedict[i] = sa.types.Float(precision=3, asdecimal=True)
                    elif dtype_str == 'int':
                        dtypedict[i] = sa.types.INT()
                    elif dtype_str == 'datetime':
                        dtypedict[i] = sa.types.DateTime()
                    else:
                        dtypedict[i] = sa.types.NVARCHAR(length=TsfItems['nvarcharLength'])
                else:
                    # Colonnes non définies → forcer NVARCHAR
                    dtypedict[i] = sa.types.NVARCHAR(length=TsfItems['nvarcharLength'])
            else:
                # Si on force tout en NVARCHAR
                dtypedict[i] = sa.types.NVARCHAR(length=TsfItems['nvarcharLength'])
        return dtypedict
                        ...
                        for file in cleanlist:
                            try:
                                ...
                                if sftpItems['port'] == 22:
                                    ...
                                    with sftp.open(file, 'rb') as remote_file, open(TsfItems['localFolder'] + '/' + fileName, 'wb') as local_file:
                                        while True:
                        ...
                        for file in cleanlist:
                            ...
                            try:
                                ...
                                if file_extension in ['.csv', '.txt', '.ows']:
                                    if 'postTransfert' in globals() and callable(globals()['postTransfert']):
                                        postTransfert(filepath, encoding = TsfItems['encoding'])
                                    if TsfItems.get('cleanCsv') == True:
                                    ...
                                    for col in df.select_dtypes(include=['float']):
                                        df[col] = df[col].apply(lambda x: format(x, '.0f') if pd.notnull(x) and float(x).is_integer() else (format(x, '.20f') if pd.notnull(x) else None))
